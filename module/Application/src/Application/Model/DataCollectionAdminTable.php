...
class DataCollectionAdminTable extends AbstractTableGateway {

    protected $table = 'data_collection_admin';
    ...
    public function addDataCollectionDetails($params){
	    ...
	    // }
			...
			//Add a new row into data collection event log table
				...
				$dataCollectionEventLogDb = new DataCollectionEventLogAdminTable($dbAdapter);
    ...
    $sQuery = $sql->select()->from(array('da_c' => 'data_collection_admin'))
...
//					->join(array('anc' => 'anc_site'), "anc.anc_site_id=da_c.anc_site",array('anc_site_name','anc_site_code'),'left')
	...
	$tQuery = $sql->select()->from(array('da_c' => 'data_collection_admin'))
                      	->join(array('u' => 'user'), "u.user_id=da_c.added_by",array('user_name'))
                      	->join(array('c' => 'country'), "c.country_id=da_c.country",array('country_name'))
		     			->join(array('t' => 'test_status'), "t.test_status_id=da_c.status",array('test_status_name'))
		      			//->join(array('anc' => 'anc_site'), "anc.anc_site_id=da_c.anc_site",array('anc_site_name','anc_site_code'),'left')
	...
	//    }
	    ...
	    if(trim($aRow['asante_rapid_recency_assy'])!= ''){
			...
			if(isset($asanteRapidRecencyAssy['rrdt'])){
				...
				$asanteRapidRecencyAssayPnLogVal = (isset($asanteRapidRecencyAssy['rrdt']['reader']))?$asanteRapidRecencyAssy['rrdt']['reader']:'';
	    ...
	    $pdfLink = '';
	    //data view
	    $dataView = '<a href="/data-collection-admin/view/' . base64_encode($aRow['data_collection_id']) . '/' . base64_encode($parameters['countryId']) . '" class="waves-effect waves-light btn-small btn blue-text custom-btn custom-btn-blue margin-bottom-1" title="View"><i class="zmdi zmdi-eye"></i> View</a>&nbsp;&nbsp;';
	    //data edit
	    if($loginContainer->hasViewOnlyAccess != 'yes' && $aRow['test_status_name']!= 'locked'){
		$dataEdit = '<a href="/data-collection-admin/edit/' . base64_encode($aRow['data_collection_id']) . '/' . base64_encode($parameters['countryId']) . '" class="waves-effect waves-light btn-small btn pink-text custom-btn custom-btn-pink margin-bottom-1" title="Edit"><i class="zmdi zmdi-edit"></i> Edit</a>&nbsp;&nbsp;';
	    }
	    //data lock
	...
	$dbAdapter = $this->adapter;
        $sql = new Sql($dbAdapter);
	if($params['type'] == 'lab'){
	    $patientQuery = $sql->select()->from(array('da_c' => 'data_collection_admin'))->columns(array('data_collection_id','status'))
		...
		if(isset($params['countryId']) && trim($params['countryId'])!= ''){
		    if($patientResult->status == 2){
		      $redirectUrl = "/data-collection-admin/view/".base64_encode($patientResult->data_collection_id)."/".$params['countryId'];
		    }else{
		      $redirectUrl = "/data-collection-admin/edit/".base64_encode($patientResult->data_collection_id)."/".$params['countryId'];
		    }
		}else if(isset($params['optCountryId']) && trim($params['optCountryId'])!= ''){
		    if($patientResult->status == 2){ 
		      $redirectUrl = "/data-collection-admin/view/".base64_encode($patientResult->data_collection_id)."/";
		    }else{
		      $redirectUrl = "/data-collection-admin/edit/".base64_encode($patientResult->data_collection_id)."/";
		    }
		}
	    }
	...
	$orderColumns = array('receipt_date_at_central_lab','da_c.patient_barcode_id','da_c.specimen_collected_date','anc.anc_site_name','anc.anc_site_code','da_c.anc_patient_id','da_c.age','da_c.gestational_age','da_c.specimen_picked_up_date_at_anc','f.facility_name','f.facility_code'/*,'da_c.lab_specimen_id'*/,'r_r.rejection_code','lab_tech_name','da_c.date_of_test_completion','da_c.result_dispatched_date_to_clinic','da_c.final_lag_avidity_odn','da_c.lag_avidity_result','da_c.hiv_rna','da_c.recent_infection','da_c.asante_rapid_recency_assy','da_c.asante_rapid_recency_assy','da_c.comments','date_result_returned_clinic','date_returned_to_participant');
       ...
       $sQuery = $sql->select()->from(array('da_c' => 'data_collection_admin'))
		     ...
		     ->join(array('anc_l_d'=>'location_details'),'anc_l_d.location_id=anc.district',array('anc_site_district'=>'location_name'),'left')
                     ->join(array('f' => 'facility'), "f.facility_id=da_c.lab",array('facility_name','facility_code'),'left')
		     ->join(array('r_r' => 'specimen_rejection_reason'), "r_r.rejection_reason_id=da_c.rejection_reason",array('rejection_code'),'left')
		     ->join(array('ussd_s' => 'ussd_survey'), "ussd_s.patient_barcode_id=da_c.patient_barcode_id",array('dateResultReturnedClinic'=>'ussd_s.date_result_returned_clinic','dateReturnedtoParticipant'=>'ussd_s.date_returned_to_participant'),'left')
				->join(array('r' => 'return_of_recency_results'), "r.patient_barcode_id=da_c.patient_barcode_id",array('ancDateResultReturnedClinic'=>new Expression('DATE(r.date_returned_to_anc)'),'ancDateReturnedtoParticipant'=>new Expression('DATE(r.date_returned_to_participant)')),'left');
	if($loginContainer->roleCode== 'LDEO' || $loginContainer->roleCode== 'LS'){
	    if(trim($parameters['lab']) ==''){
	       $sQuery = $sQuery->where('da_c.lab IN ("' . implode('", "', $mappedLab) . '")');
	...
	if($loginContainer->roleCode == 'ANCSC' && trim($parameters['anc']) == ''){
        ...
        }else if(($loginContainer->roleCode== 'LDEO' || $loginContainer->roleCode== 'LS') && trim($parameters['lab']) == ''){
	    $sQuery = $sQuery->where('da_c.lab IN ("' . implode('", "', $mappedLab) . '")');
	}
	if(isset($parameters['countryId']) && trim($parameters['countryId'])!= ''){
	    $sQuery = $sQuery->where(array('da_c.country'=>$parameters['countryId']));
	...
	//    }
	    ...
	    $row = array();
	    $row[] = '<input type="checkbox" class="filled-in" name="data-select[]" id="'.$aRow['data_collection_id'].'" value="'.base64_encode($aRow['data_collection_id']).'" onchange="doMultipleDataSelect(this);"/><label for="'.$aRow['data_collection_id'].'" style="padding-left:40%;"></label>';
	    $row[] = $aRow['patient_barcode_id'];
	    $row[] = $specimenCollectedDate;
	    $row[] = $status;
	    $row[] = ucwords($aRow['anc_site_name']);
	...
	if($showLabRecent){
	    $labRecentQuery = $sql->select()->from(array('anc' => 'anc_site'))
				  ...
				  ->join(array('da_c'=>'data_collection_admin'),'da_c.anc_site=anc.anc_site_id',array('noofLAgRecent' => new \Zend\Db\Sql\Expression("SUM(IF(da_c.recent_infection = 'yes', 1,0))"),'noofLabRecencyAssayRecent' => new \Zend\Db\Sql\Expression('SUM(IF(da_c.asante_rapid_recency_assy like \'%rrr":{"assay":"absent"%\', 1,0))')),'left')
	...
	if($showANCRecent){
	    $ancRecentQuery = $sql->select()->from(array('anc' => 'anc_site'))
				  ...
				  ->join(array('anc_r_r'=>'anc_rapid_recency'),'anc_r_r.assessment_id=r_a.assessment_id',array('noofANCRecencyTestRecent' => new \Zend\Db\Sql\Expression("SUM(IF(anc_r_r.recency_line = 'recent', 1,0))")),'left')
				  ->join(array('da_c'=>'data_collection_admin'),'da_c.patient_barcode_id=r_a.patient_barcode_id',array(),'left')
	...
	$s_t_start_date = '';
	   ...
	   if(isset($s_t_date[0]) && trim($s_t_date[0]) != "") {
        ...
        $dbAdapter = $this->adapter;
        $sql = new Sql($dbAdapter);
        $select1 = $sql->select()->from(array('da_c' => 'data_collection_admin'))
	...
	$select2 = $sql->select()->from(array('da_c' => 'data_collection_admin'))
	...
	foreach ($rResult as $aRow) {
	    ...
	    if(isset($aRow['test_status_name']) && $aRow['test_status_name']!= null && trim($aRow['test_status_name'])!= ''){
	       $status = '<a href="/data-collection-admin/view/' . base64_encode($aRow['data_collection_id']) . '/' . base64_encode($aRow['country']) . '"target="_blank" title="View data"> '.ucfirst($aRow['test_status_name']).'</a>';
	    }
	    //LAg assay
	...
	$dbAdapter = $this->adapter;
        $sql = new Sql($dbAdapter);
	$collectionQuery = $sql->select()->from(array('da_c' => 'data_collection_admin'))
				->columns(array(
	...
	while($start <= $end){
	    $month = date('m', $start);$year = date('Y', $start);$monthYearFormat = date("M-Y", $start);
            $query = $sql->select()->from(array('da_c'=>'data_collection_admin'))
                          ->columns(
                                  array(
